...
/**
 * Maps [AudioSessionMode] to the ExoPlayer audio attributes applied to the actual
 * output stream (routing, volume curve, Bluetooth profile).
 *
 * The focus-arbitration counterpart (`android.media.AudioAttributes`, used by
 * `AndroidAudioSession`) lives in `:core` as
 * `kplayer.core.audio.platformAudioAttributesFor`. The two must agree about what
 * kind of content is playing — [AudioSessionMode] is the shared contract that
 * keeps them in step, so a new mode must be handled in both places.
 */
...
internal fun exoAudioAttributesFor(mode: AudioSessionMode): ExoPlayerAudioAttributes =
